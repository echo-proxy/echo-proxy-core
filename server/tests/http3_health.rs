...
async fn get_health_returns_200_via_h3_client() {
...

    {
        ...
        assert_eq!(body, b"ok\n");
        assert!(
            stream.recv_data().await.expect("recv_data_eof").is_none(),
            "response body should end cleanly (no reset after ok\\n)"
        );
    }
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_unknown_returns_404_via_h3_client() {
    ensure_crypto_provider();
    ...
    let uri: http::Uri = format!("https://localhost:{port}/no-such-path")
        .parse()
        .expect("URI");
...

    {
        ...
        assert!(
            stream.recv_data().await.expect("recv_data_eof").is_none(),
            "404 response should end cleanly"
        );
    }
