...
pub async fn server(addr: String, user_list: Vec<String>) {
...
pub async fn accept_connection(stream: TcpStream, user_list: Vec<String>) {
...
pub async fn process_control(mut ws_stream: WebSocketStream<TcpStream>, user_list: Vec<String>) {
...
pub async fn exchange_data(ws_stream: WebSocketStream<TcpStream>, dist: String) {
...
mod tests {
    use super::*;
    // use async_tungstenite::tungstenite::http::header::HeaderValue;
    // use async_tungstenite::tungstenite::http::Request;
    use semver::Version;

    #[test]
    fn test_version_check_supported() {
        let required_semver = VersionReq::parse(SUPPORTED_VERSION).unwrap();
        let installed_semver = Version::parse("0.1.0").unwrap();

        assert!(required_semver.matches(&installed_semver));
    }

    #[test]
    fn test_version_check_unsupported() {
        let required_semver = VersionReq::parse(SUPPORTED_VERSION).unwrap();
        let installed_semver = Version::parse("0.0.9").unwrap();

        assert!(!required_semver.matches(&installed_semver));
    }

    #[test]
    fn test_user_authentication_valid() {
        let user_list = vec!["user1".to_string(), "user2".to_string()];
        let test_user = "user1".to_string();

        assert!(user_list.contains(&test_user));
    }

    #[test]
    fn test_user_authentication_invalid() {
        let user_list = vec!["user1".to_string(), "user2".to_string()];
        let test_user = "user3".to_string();
    ...
    fn test_parse_hello_message() {
        let message = "Hello:0.1.0:user1";
        let parts: Vec<&str> = message.split(":").collect();

        assert_eq!(parts.len(), 3);
        assert_eq!(parts[0], "Hello");
        assert_eq!(parts[1], "0.1.0");
        assert_eq!(parts[2], "user1");
    }

    #[tokio::test]
    async fn test_process_control_message_format() {
        // 测试消息格式解析
        let message = "Hello:0.1.0:user1";
        let parts: Vec<&str> = message.split(":").collect();
    ...
    fn test_uuid_generation() {
        ...
        // UUID应该是有效的
        assert!(token1.to_string().len() > 0);
        assert!(token2.to_string().len() > 0);
    }
